---
title: "Security and Privacy in Computing"
collection: teaching
type: "Graduate/Undergraduate course"
permalink: /teaching/2024-security-privacy
venue: "Johns Hopkins University, Department of Computer Science"
date: 2024-01-01
location: "Baltimore, MD"
---

Lecturer for the Security and Privacy in Computing course, covering hardware and software security design, vulnerability assessment and reverse engineering, hardware and software-based attacks on computer components including RAM and CPUs, applied cryptography, computer architecture, networking, and component-level analysis. The course uses hardware and software emulation and virtualization techniques to present students with real-world environments for hands-on projects.
