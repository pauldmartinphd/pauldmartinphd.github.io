...
One line of research now ties that work to what he is building today: software supply-chain vulnerability assessment, and the widening gap between the version a software component appears to run and the vulnerabilities it is actually exposed to. Firmware IQ identifies components and versions and maps them to known CVEs — a premise Dr. Martin's current research stress-tests one layer up, at the operating-system distribution. Enterprise and "stable" Linux distributions maintain soft forks of upstream packages, with incomplete backports, bespoke patches, and out-of-sync LTS trees, so two packages reporting the same upstream version can carry very different real-world exposure. He is quantifying this vulnerability divergence, and the patch lead time between rolling and enterprise distributions, to ask whether rolling distributions are in fact more secure, whether enterprise distributions should track rolling LTS packages, and whether distributions should converge on shared soft-forks so maintenance effort can be pooled rather than duplicated. Those findings in turn motivate the Linux distribution he is currently building, whose patch-management model is designed around what the divergence measurements reveal.
